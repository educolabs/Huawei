...
HW_Script_SetApprmDatToFile()
{
	...
	then
		...
		if [ 0 -eq $? ]
		then
			cfgtool set $var_default_ctree $var_node_apprm LocalUserPassword $var_wpa
			if [ 0 -ne $? ]
			then
				echo "Failed to set X_HW_AppRemoteManage LocalUserPassword!"
				return 1
			fi
		fi
		...
		if [ 0 -eq $? ]
		then
			cfgtool set $var_default_ctree $var_node_apprm LocalAdminPassword $var_wpa
			if [ 0 -ne $? ]
			then
				echo "Failed to set X_HW_AppRemoteManage LocalAdminPassword!"
				return 1
	        fi
		fi
	fi
