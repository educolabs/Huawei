...
{
	read -r var_userPwd var_ssid var_wpa var_ssid2 var_wpa2 < $var_customize_file
    if [ 0 -ne $? ]
    then
	...
	#存在/mnt/jffs2/V5_TypeWord_FLAG 文件不校验密码长度
...
HW_Script_GetOperatorIdByBinCfgKey()
{
    ...
    read var_g_Binword var_g_Cfgword < $var_jffs2_customize_txt_file
    if [ 0 -ne $? ]
    then
		var_OperatorId="CTC"
        return
