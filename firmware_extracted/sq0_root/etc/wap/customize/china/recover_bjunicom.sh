...
{		
	...
	echo $var_boardinfo_temp | xargs sed 's/obj.id = \"0x00000019\" ; obj.value = \"[A-Za-z0-9-]*\"/obj.id = \"0x00000019\" ; obj.value = \"'$var_specsn'\"/g' -i
...
HW_Script_SetDatToFile()
{
    ...
    then
        ...
        # set ssid2 
    cfgtool set $var_default_ctree $var_node_ssid2 SSID "GUEST_$var_para"
    if [ 0 -ne $? ]
    then
    ...
    # set ssid3 
    cfgtool set $var_default_ctree $var_node_ssid3 SSID "STB_$var_para"
    if [ 0 -ne $? ]
    then
    ...
    if [ 0 -ne $? ]
    then
        echo "Failed to set common ssid3 wap password!"
        return 1
    fi
    
    # set ssid4 
    cfgtool set $var_default_ctree $var_node_ssid4 SSID "BACKUP"
    if [ 0 -ne $? ]
    then
    ...
    if [ 0 -ne $? ]
    then
        echo "Failed to set common ssid4 wap password!"
        return 1
    fi
