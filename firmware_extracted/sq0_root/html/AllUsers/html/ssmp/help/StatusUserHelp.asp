...
var BJUNICOMFlag = '<%HW_WEB_GetFeatureSupport(BBSP_FT_BJUNICOM);%>';
var HardStat = '<%HW_WEB_GetFeatureSupport(FT_USERDEV_TRAFFIC_HARD_STAT);%>';
stProductCapInfo = GetProductCapInfo();

if (('1' == BJUNICOMFlag) && ('1' == HardStat))
{
	if (stProductCapInfo.wlan == 0 && stProductCapInfo.usb == 0)/* 没有WLAN，没有usb */
	{
		document.write("<p><b>用户侧信息</b></p><p>包括家庭终端用户侧以太网接口信息,以太网用户信息。</p>");
	}
	else if (stProductCapInfo.wlan == 0 && stProductCapInfo.usb == 1)/* 没有WLAN，有usb */
	{
		document.write("<p><b>用户侧信息</b></p><p>包括家庭终端用户侧以太网接口信息，以太网用户信息，USB接口信息，。</p>");
	}
	else if(stProductCapInfo.wlan == 1 && stProductCapInfo.usb == 0)/* 有WLAN，没有usb */
	{
		document.write("<p><b>用户侧信息</b></p><p>包括家庭终端用户侧WLAN接口信息，WLAN用户信息，以太网接口信息，以太网用户信息。</p>");
	}
	else if (stProductCapInfo.wlan == 1 && stProductCapInfo.usb == 1)/* 有WLAN，有usb */
	{
		document.write("<p><b>用户侧信息</b></p><p>包括家庭终端用户侧WLAN接口信息，WLAN用户信息，以太网接口信息，以太网用户信息，USB接口信息。</p>");
	}
}
else
{
