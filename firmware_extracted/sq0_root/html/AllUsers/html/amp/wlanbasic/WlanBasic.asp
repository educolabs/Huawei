...
if (CfgMode.toUpperCase() == 'DBAA1') {
    for (var i = 0; i < g_keys.length - 1; i++) {
        if (g_keys[i].value == "aaaaaaaaaaaaa") {
    ...
    switch (authMode)
    {
        ...
        case 'wpa2/wpa3-psk':
            ...
            if ((authMode == "wpa3-psk") || (authMode == "wpa2/wpa3-psk")) {
                ...
                document.getElementById("hidewlWpaPsk_note").innerHTML = cfg_wlancfgdetail_language['amp_wpa_psknote' + ('1' == kppUsedFlag ? '_63' : '')];
                if (PccwFlag == 1) {
                    addEncryMethodOption(authMode);
                } else {
            ...
            if (preflag == 1) {
                document.getElementById("hidewlWpaPsk_note").innerHTML  = cfg_wlancfgdetail_language['amp_wpa_psknote_pldt'];
            } 
...
function IsNotSupportApply() {
    var curWlanInst = getWlanInstFromDomain(Wlan[ssidIdx].domain);
    if ((aisAp == 1) && (DoubleFreqFlag == 1) && (wlanpage == '5G') && (curWlanInst == 8) && (curUserType == sptUserType)){
        return true;
    }
...
function SetDomDisabled() {
    if ((aisAp != 1) || (DoubleFreqFlag != 1) || (wlanpage != '5G')) {
        return;
    }
...
function SetSsidDisabled() {
    ...
    if (aisApBackhaul == 1) {
        if (((curWlanInst == 4) || (curWlanInst == 8)) && (aisapBSDInfo.enableSSID4_8 == 1)) {
            configSsidFlag = 1;
        }
    } else {
        if ((curWlanInst == 8) && (curUserType == sptUserType)) {
            configSsidFlag = 1;
        }
    }
...
function showPSKwarningSug()
{
	...
	if (IsAuthModePsk(AuthMode))
	{
		if(((0 == BaseInstFacKeyChgF2g)&&("2G" == wlanpage))
		|| ((0 == BaseInstFacKeyChgF5g)&&("5G" == wlanpage)))
		{
...
function selectLine(id)
{
	...
	if ((1 == DoubleFreqFlag) && ("5G" == wlanpage) && (uiTotal5gNum > 0) && (1 == OIFlag))
	{
...
function setControl(idIndex)
{
    ...
    else
    {
		...
		if(1 == IsPtVdf)
		{
			...
			if (IsAuthModePsk(AuthMode))
			{
				if(((0 == BaseInstFacKeyChgF2g)&&(1 == wlanInst))
				|| ((0 == BaseInstFacKeyChgF5g)&&(5 == wlanInst)))
				{
					setDisplay("pskpasswordwarning",1);
					setPSKwarningSug();
				}
...
var WlanBasicSummaryArray = new Array(new stSummaryInfo("text",wlanbasic_note),
    ...
    if ((CfgMode.toUpperCase() == 'CTMMO') && (TypeWord == 'BRIDGE')) {
        WlanBasicSummaryArray = new Array(new stSummaryInfo("text",wlanbasic_note),
                                    new stSummaryInfo("img","../../../images/icon_01.gif", GetDescFormArrayById(cfg_wlancfgother_language, "amp_wlan_notechmk")),
                                    new stSummaryInfo("text","<span style='color:red;'>If you are interested in our Wi-Fi service, please contact with operator for more details.</span>"),
                                    null);
    }
      ...
      <tr class="head_title">
        ...
        {   
            ...
            if ((1 == DoubleFreqFlag) && ("5G" == wlanpage))
            {
                ...
                if ((aisApBackhaul == 1) && (WlanMap[i].portIndex > (basic5GIfIndex+3))) {
                    continue;
                }
            ...
            if ((1 == DoubleFreqFlag) && ("2G" == wlanpage))
            {
                ...
                if ((aisApBackhaul == 1) && (WlanMap[i].portIndex > 3)) {
                    continue;
                }
            }
            ...
            if (   ( 0 == WlanMap[i].portIndex ) 
                || ( (basic5GIfIndex == WlanMap[i].portIndex) && (1 == DoubleFreqFlag) ) 
                || ( (1 == IspSSIDVisibility) && (1 == isSsidForIsp(curWlanInst)) )  
				|| ((IsGuestSsid(curWlanInst)) && (1 == t2Flag)))
            {
		...
		<tr id='tr_sta_isolation' style='display:none;'>
            <td class="table_title width_per25">
                <script>
                    ...
                    if ((CfgMode.toUpperCase() == 'TEDATA') || (CfgMode.toUpperCase() == 'TEDATA2')) {
