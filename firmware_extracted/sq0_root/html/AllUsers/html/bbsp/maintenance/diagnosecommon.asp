...
function OnApply()
{
  ...
  }else
  {
    ...
    setNoEncodeInnerHtmlValue('PingTestTitle', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_testing']+ '</FONT><B>');
...
{   
     ...
     else if( status.indexOf("Complete") >= 0)
     {
        ...
        setNoEncodeInnerHtmlValue('PingTestTitle', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_result']+ '</FONT><B>');
        showPingDnsInfo(diagnose_language['bbsp_dnstitle'], replaceSpace(dnsString));
        setNoEncodeInnerHtmlValue('PingTitle', diagnose_language['bbsp_pingtitle']);
...
function LoadFrame()
{
    ...
    else if  (CLICK_TERMINAL_FLAG == TracerouteClickFlag)
    {
        ...
        {
            setNoEncodeInnerHtmlValue('traceRouteresult', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_testing']+ '</FONT><B>');
        }
        setDisable('btnTraceroute',0);
        setDisable("btnStopTraceroute", 1);     
    }
    ...
    {
        setNoEncodeInnerHtmlValue('equipTestResult', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['selftestwait']+ '</FONT><B>');
    ...
    if(CLICK_START_FLAG == PingClickFlag) {
     ...
     else if(CLICK_TERMINAL_FLAG == PingClickFlag)
     {
        ...
        {
            setNoEncodeInnerHtmlValue('PingTestTitle', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_stopping']+ '</FONT><B>');
    ...
    if(ProductType == '2')
    {
        ...
        {
            setDisplay('table_ping_title',0);
            setDisplay('table_ping',0);
            setDisplay('TraceRoute',0);
        }
...
function WriteOptionFortraceRoute()
{
       InitWanNameListControl2("wanname", function(item){
        ...
        else if ((curUserType != sysUserType) && ("1" == GetCfgMode().DT_HUNGARY))
        {
            ...
            if ((item.Mode == 'IP_Routed') && (item.Enable == 1) && (item.Tr069Flag == '0') &&
                (item.ServiceList.toString().toUpperCase() != "TR069") &&
                (item.ServiceList.toString().toUpperCase() != "VOIP") &&
                (item.ServiceList.toString().toUpperCase() != "TR069_VOIP")) {
                return true;
            }
        }
        else
...
{   
     ...
     else if( status.indexOf("Complete") >= 0)
     {
        ...
        setNoEncodeInnerHtmlValue('traceRouteresult', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_result']+ '</FONT><B>');
...
function startNSlookup()
{
    with (getElement('NsLookupForm'))
    {
        ...
        setDisable('nslookup_wanname',1);
        setDisable('btnNsLookupStart',1);
        setNoEncodeInnerHtmlValue('nslookup_process', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_testing']+ '</FONT><B>');
            ...
            success : function(data) {
                ...
                if(NslookupOpInfoList != null) 
                {
                    ...
                    {    
                        ...
                        setNoEncodeInnerHtmlValue('nslookup_process', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_result']+ '</FONT><B>');
...
if (ProductType != '3')
{
    ...
    function EquipTestResult()
    {
        ...
        setNoEncodeInnerHtmlValue('equipTestResult', '<B><FONT color=' + titleColorRef + '>' +diagnose_language['bbsp_result']+ '</FONT><B>');
...
<script language="JavaScript" type="text/javascript">
    ...
    if (CfgModeWord.toUpperCase() == "DESKAPASTRO") {
        if (ApModeValue == '2' || ApModeValue == '3') {
            setNoEncodeInnerHtmlValue("DCpingtitle_content", diagnose_language["bbsp_diagnose_title1_astro"]);
        } else {
...
<div id="mainend" style="display:none;"> 
  ...
  <script>
    ...
    function IsValidWan(Wan)
    {
            ...
            && ((CfgModeWord.toUpperCase() == "RDSGATEWAY") || ("1" == GetCfgMode().DT_HUNGARY)))
        {
            ...
            if ((Wan.Mode == "IP_Routed") && (Wan.Enable == 1) &&
                (Wan.ServiceList.toString().toUpperCase() != "TR069") &&
                (Wan.ServiceList.toString().toUpperCase() != "VOIP") &&
                (Wan.ServiceList.toString().toUpperCase() != "TR069_VOIP")) {
                return true;
            }
        }
        else
    ...
    function ShowPingResult()
    {
        ...
        setNoEncodeInnerHtmlValue('PingResult', ((parseInt(PingResult.SuccessCount,10) > 0) ? ("<B><FONT color=" + titleColorRef + ">" + diagnose_language['bbsp_pingpass'] + "</FONT><B>") : ("<B><FONT class='color_red'>" + diagnose_language['bbsp_pingfail'] + "</FONT><B>")));
    }
