...
function gotoGuidePage() {
    ...
    if ((CfgMode.toUpperCase() == 'TURKCELL2') && (ProductType != '2') && (pwdModifyFlag != '0')) {
        return;
    }
...
function loadframe()
{
    if (CfgMode.toUpperCase() == 'ORANGEMT2') {
        $("#headerbg").css("background", "url(../../images/header_ORANGEMT.jpg) center center / cover no-repeat");
	...
	if ((IsSupportBridgeWan == 0) || ((IsSupportBridgeWan == 1) && (ProductType == '1')))
	{
        ...
        if(parseInt(logo_singtel, 10) != 1)
        {
            ...
            if (TedataGuide == 1) {
                if (TypeWord_com == 'ACUD') {
                    $("#headerbrandlog").css("background", "url(images/logo_aucd.gif) no-repeat center");
                    $("#headerbrandlog").css("width", "60PX");
                } else {
...
function updateSignalStatus() {
    ...
    var signalCategory = [["GSM", 18, 21, 23, 26, 96], ["WCDMA", 9, 9, 16, 23, 96], ["TD-SCDMA", 7, 7, 10, 13, 96],
                        ["LTE", 26, 26, 36, 46, 96], ["CDMA", 21, 21, 26, 31, 96], ["EVDO", 16, 16, 26, 36, 96]];
    ...
    for (var i = 0; i < signalCategory.length; i++) {
        if (systemMode == signalCategory[i][0]) {
            if (signalQuality < signalCategory[i][1]) {
                signal = 0;
            } else if ((signalQuality >= signalCategory[i][2]) && (signalQuality < signalCategory[i][3])) {
                signal = 1;
            } else if ((signalQuality >= signalCategory[i][3]) && (signalQuality < signalCategory[i][4])) {
                signal = 2;
            } else if ((signalQuality >= signalCategory[i][4]) && (signalQuality < signalCategory[i][5])) {
                signal = 3;
...
if ((CfgMode.toUpperCase() == 'TTNET2') && (IsModifiedPwd == 0) && (flagTips)) {
        flagTips = false;
        var accountUrl = "/html/ssmp/accoutcfg/account.asp"
        if (curUserType == sysUserType) {
